...
public class BudgetsController {
	...
	public void initialize() throws IOException {
		...
		maintenanceRequests
				.forEach(maintenanceRequest -> selectRequestCombobox.getItems().add(maintenanceRequest.getRequestId()));
	...
	public void onCreateBudgetButtonClick(ActionEvent actionEvent) throws IOException {
		...
		Optional<RestockSuppliesRequest> findRestockSuppliesRequest = restockSuppliesRequests.stream()
				.filter(restockSuppliesRequest -> restockSuppliesRequest.getRequestId() == selectRequestCombobox
						.getValue())
				.findFirst();
		...
		if (findRestockSuppliesRequest.isPresent()) {
			...
			try {
				StreamMapper stream = new StreamMapper();
				ObjectOutputStream oos2 = getObjectOutputStream(stream.getObjectStream(ObjectStreams.BudgetObjects),
						false);
				ObjectOutputStream oos1 = getObjectOutputStream(
						stream.getObjectStream(ObjectStreams.RestockSuppliesRequestObjects), true);
