...
public class RegulationsController {
	...
	public void loadRegulations() throws IOException {
	...
	@javafx.fxml.FXML
	public void onCreateNewRegulationButtonClick(ActionEvent actionEvent) throws IOException {
		...
		if (!Regulation.isValidEffectiveDate(effectiveDatePicker.getValue())) {
			Utilities.showAlert("Effective date must be in the future", Alert.AlertType.ERROR);
			return;
		}
		...
		ruleSummaryInput.clear();
		ruleDescriptionInput.clear();
		effectiveDatePicker.setValue(null);
		regulationTypeCombobox.setValue(null);
		...
		try {
			StreamMapper stream = new StreamMapper();
			ObjectOutputStream oos = getObjectOutputStream(stream.getObjectStream(ObjectStreams.RegulationObjects),
					false);
