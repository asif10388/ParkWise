...
public class OnsiteRegistrationsComtroller {
	...
	public void initialize() throws IOException {
		...
		campsites.forEach(campsite -> {
			if (!campsite.isCampsiteUnderMaintenance()) {
				selectCampsiteCombobox.getItems().add(campsite.getCampsiteName());
			}
		});
	}
		...
		if (findCampsite.isPresent()) {
			...
			try {
				...
				ObjectOutputStream oos = getObjectOutputStream(
						stream.getObjectStream(ObjectStreams.CampsiteBookingObjects), false);
	...
	public void onCheckOutDateValueSelected(ActionEvent actionEvent) throws IOException {
		...
		if (findCampsite.isPresent()) {
			...
			double getPrice = findCampsite.get().calculateTotalFee(days);
