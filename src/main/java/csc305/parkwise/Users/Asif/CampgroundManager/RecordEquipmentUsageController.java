...
public class RecordEquipmentUsageController {
	...
	public void loadEquipmentTable() throws IOException {
	...
	public void onSubmitUsageButtonClick(ActionEvent actionEvent) throws IOException {
		...
		List<Equipment> equipments = getEquipmentsFromFile();
		Optional<Equipment> findEquipment = equipments.stream().filter(obj -> obj.getItemName().equals(itemName))
				.findFirst();
		...
		if (findEquipment.isPresent()) {
			...
			if (selectedActionType.equals("check-out")
					&& findEquipment.get().isItemAvailabilityStatus().equals("Available")) {
			...
			try {
				StreamMapper stream = new StreamMapper();
				ObjectOutputStream oos = getObjectOutputStream(stream.getObjectStream(ObjectStreams.EquipmentObjects),
						true);
